...
class SwipeController: NSObject {
    ...
    func configureActionsView(with actions: [SwipeAction], for orientation: SwipeActionsOrientation) {
        ...
        actionsView.layer.cornerRadius = options.actionsViewCornerRadius ?? 0

        actionsContainerView.addSubview(actionsView)
        
        let topInset = options.actionsViewInsets?.top ?? 0
        let bottomInset = options.actionsViewInsets?.bottom ?? 0
        let rightInset = options.actionsViewInsets?.right ?? 0
        let leftInset = options.actionsViewInsets?.left ?? 0
        
        actionsView.bottomAnchor.constraint(equalTo: swipeable.bottomAnchor, constant: -bottomInset).isActive = true
        actionsView.widthAnchor.constraint(equalTo: swipeable.widthAnchor, multiplier: 2).isActive = true
        actionsView.topAnchor.constraint(equalTo: swipeable.topAnchor, constant: topInset).isActive = true
        
        if orientation == .left {
            actionsView.rightAnchor.constraint(equalTo: actionsContainerView.leftAnchor, constant: -rightInset).isActive = true
        } else {
            actionsView.leftAnchor.constraint(equalTo: actionsContainerView.rightAnchor, constant: leftInset).isActive = true
        }
